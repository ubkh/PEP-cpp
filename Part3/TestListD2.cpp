...
int main() {
    ...
    
    {
        ...
        vector<int> numbersOut2;
        
        for (auto & n : aList) { // uses begin() and end() to loop over aList
            numbersOut2.push_back(n);
        }
        
        vector<int> shouldBe2{3,4,6,8};

        if (numbersOut2 == shouldBe2) {
            cout << "4) Pass: making a list for {2,4,6,8} using an initializer list, then inserting 3 before 4, then erasing 2, gives the list {3,4,6,8}\n";
        } else {
            ++retval;
            cout << "4) Fail: making a list for {2,4,6,8} using an initializer list, then inserting 3 before 4, then erasing 2, should give the list {3,4,6,8} but instead produced the list: {";
